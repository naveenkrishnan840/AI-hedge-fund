...
def analyze_consistency(financial_line_items: list) -> dict[str, any]:
    ...
    earnings_values = [item["net_income"] for item in financial_line_items if item.get("net_income")]
...
def generate_buffett_output(
    ...
    #     with_structured_output(schema=WarrenBuffettSignal)
    llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash").with_structured_output(schema=WarrenBuffettSignal)
    # llm = (ChatOpenAI(base_url=os.getenv("OPENROUTER_BASE_URL"), model=os.getenv("MODEL_NAME")).
           # with_structured_output(schema=WarrenBuffettSignal, method="json_mode"))
