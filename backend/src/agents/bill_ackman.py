...
def analyze_business_quality(metrics: list, financial_line_items: list) -> dict:
    ...
    revenues = [item["revenue"] for item in financial_line_items if item.get("revenue") is not None]
    if len(revenues) >= 2:
    ...
    fcf_vals = [item["free_cash_flow"] for item in financial_line_items if item.get("free_cash_flow") is not None]
    op_margin_vals = [item["operating_margin"] for item in financial_line_items if item.get("operating_margin") is not None]
...
def generate_ackman_output(
    ...
    llm = ChatGoogleGenerativeAI(model="gemini-2.0-flash").with_structured_output(schema=BillAckmanSignal)
    # llm = (ChatOpenAI(base_url=os.getenv("OPENROUTER_BASE_URL"), model=os.getenv("MODEL_NAME")).
    #        with_structured_output(schema=BillAckmanSignal, method="json_mode"))
